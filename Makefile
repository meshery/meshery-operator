...
operator-manifest: manifests kustomize ## Re-render config/manifests/default.yaml (the flat operator install manifest mesheryctl fetches over HTTP) from config/default.
	@printf '%s\n' \
		'# Code generated by "make operator-manifest" (kustomize build config/default). DO NOT EDIT.' \
		'# Flat, self-contained operator install manifest fetched over HTTP by mesheryctl' \
		'# (OperatorURL in meshery/meshery -> config/manifests/default.yaml on master). Meshery' \
		'# Server installs the operator via its Helm chart, not this file.' \
		'# To change: edit the config/ kustomize tree, then re-run make operator-manifest. A CI' \
