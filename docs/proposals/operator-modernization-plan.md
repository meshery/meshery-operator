...
| 9 | M | `MeshSyncSpec.Version` is **unused** — the Deployment image is always `meshery/meshsync:stable-latest`, so the declared version is silently ignored. **Resolved:** `resolveVersion` honours `spec.version` (normalising a v-prefixed semver to the bare tag the registry publishes) and defaults to a pinned MeshSync release; see [release-process.md § Pinned images](../release-process.md#pinned-images). | `meshsync_types.go:42` vs `pkg/meshsync/resources.go:89` |
